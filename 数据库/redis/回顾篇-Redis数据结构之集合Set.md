...
因为intset是有序的，在升级操作中，只要知道最后一个添加的元素的字节类型就知道要升级到哪个字节，但是如果是降级，就需要一个一个遍历操作获取元素的字节类型，由此可知，intset不支持降级的操作。

添加元素的流程图

![](img/redis_intset添加流程.png)
